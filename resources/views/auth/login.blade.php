...
@section('title', 'Login - BatasKota Coffee')
...
<main class="h-screen flex items-center justify-center p-2 sm:p-4 overflow-hidden">
    <div class="w-full max-w-[1100px] h-[95vh] flex flex-col lg:flex-row bg-white dark:bg-[#2c2216] rounded-xl shadow-xl overflow-hidden border border-[#e7ddcf] dark:border-neutral-800">
        ...
        <div class="hidden lg:flex w-1/2 bg-cover bg-center relative" style="background-image: url('https://lh3.googleusercontent.com/aida-public/AB6AXuDZImU3DErTtzbzZAIFjAfblmgW_Qk0JVEBJ6Rj6gJ-7HK6ENyuDZURIO6evcMw0L_7aT-IxfvmXGjWYauS-gw2u1gGDqqPRdzXH0YcqPbk7kZ8YzECCsfkB0ZoFAtxICnm5RH9cJViOaoVqh35r1xmFJPYnMOuR3HYagWr3zfoY4g28Ahb2Qj3UyZlBIa2zjPV9S1ylXsHQD5YnSCdhoXYDzabaDBo1PQv1aVrJczpslbcAiQKGGEwj5adXGMx67oHDXg1QEY-_7s');">
            <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-8">
                <h1 class="text-white text-3xl font-black leading-tight mb-3 tracking-tight">
                    Nikmati Kopi Lokal dengan Layanan Cepat & Praktis.
                </h1>
                <p class="text-white/90 text-base font-medium">
                    Masuk untuk memesan kopi dan toast favoritmu, nikmati layanan grab & go, dan dukung UMKM lokal BatasKotaCoffee.
                </p>
            </div>
        </div>
        ...
        <div class="w-full lg:w-1/2 flex flex-col bg-white dark:bg-[#1e170e] overflow-hidden">
            ...
            <div class="flex-1 flex flex-col justify-center px-6 sm:px-12 py-6 overflow-y-auto">
                ...
                <form action="{{ route('login') }}" method="POST" class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <div class="relative group">
                            <input name="email" value="{{ old('email') }}" class="w-full rounded-lg border border-[#e7ddcf] dark:border-neutral-700 bg-[#fcfaf8] dark:bg-neutral-800/50 h-11 px-4 pl-10 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all placeholder:text-hover/60 dark:placeholder:text-neutral-600 text-[#1b160d] dark:text-white" placeholder="name@example.com" type="email" required />
                            <span class="material-symbols-outlined absolute left-3 top-2.5 text-hover group-focus-within:text-primary transition-colors text-xl">mail</span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5" x-data="{ showPassword: false }">
                        ...
                        <div class="relative group">
                            <input name="password" :type="showPassword ? 'text' : 'password'" class="w-full rounded-lg border border-[#e7ddcf] dark:border-neutral-700 bg-[#fcfaf8] dark:bg-neutral-800/50 h-11 px-4 pl-10 pr-10 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all placeholder:text-hover/60 dark:placeholder:text-neutral-600 text-[#1b160d] dark:text-white" placeholder="Masukkan password" required />
                            <span class="material-symbols-outlined absolute left-3 top-2.5 text-hover group-focus-within:text-primary transition-colors text-xl">lock</span>
                            <button type="button" @click="showPassword = !showPassword" class="absolute right-3 top-2.5 text-hover hover:text-primary transition-colors">
                    ...
                    <!-- Submit Button -->
                    <button type="submit" class="mt-2 w-full h-11 bg-primary flex justify-center items-center hover:bg-hover text-white rounded-lg font-bold text-sm tracking-wide transition-all shadow-md transform active:scale-[0.99]">
                        Login
                    </button>
                    ...
                    <div class="flex w-full justify-center">
                        <p class="text-xs text-[#1b160d] dark:text-gray-300">
                            Belum punya akun?
                            <a href="{{ route('register') }}" class="font-bold text-primary hover:text-hover">Daftar</a>
                        </p>
                    </div>
                </form>
            </div>
