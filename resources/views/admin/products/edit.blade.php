...
@section('title', 'Edit Produk - Admin BatasKota')
...
<div class="flex-1 p-8 max-w-3xl mx-auto w-full">
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Produk</h2>
    </div>
    ...
    <div class="bg-white dark:bg-[#1a140c] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                <!-- Image Preview -->
                <div x-data="{ preview: '{{ $product->image ? asset('storage/' . $product->image) : '' }}' }">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" class="w-full rounded-lg border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-3 text-slate-900 dark:text-white focus:border-primary focus:ring-1 focus:ring-primary" required />
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $product->price) }}" min="0" class="w-full rounded-lg border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-3 text-slate-900 dark:text-white focus:border-primary focus:ring-1 focus:ring-primary" required />
                </div>

                <!-- Description -->
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Deskripsi</label>
                    <textarea name="description" rows="3" class="w-full rounded-lg border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-3 text-slate-900 dark:text-white focus:border-primary focus:ring-1 focus:ring-primary">{{ old('description', $product->description) }}</textarea>
                </div>
                ...
                <div class="border-t border-slate-200 dark:border-slate-700 pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <label class="flex items-center gap-3 p-4 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                            <input type="checkbox" name="has_strength" value="1" {{ $product->has_strength ? 'checked' : '' }} class="rounded border-slate-300 text-primary focus:ring-primary" />
                            <div>
                        ...
                        <!-- Size -->
                        <label class="flex items-center gap-3 p-4 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                            <input type="checkbox" name="has_size" value="1" {{ $product->has_size ? 'checked' : '' }} class="rounded border-slate-300 text-primary focus:ring-primary" />
                            <div>
                        ...
                        <!-- Shot -->
                        <label class="flex items-center gap-3 p-4 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                            <input type="checkbox" name="has_shot" value="1" {{ $product->has_shot ? 'checked' : '' }} class="rounded border-slate-300 text-primary focus:ring-primary" />
                            <div>
                ...
                <div class="border-t border-slate-200 dark:border-slate-700 pt-6">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ $product->is_active ? 'checked' : '' }} class="rounded border-slate-300 text-primary focus:ring-primary" />
